...
- Services can be excluded from automatic registration by adding an `[Exclude(Feature.ServiceDiscovery)]` attribute to the DTO, or setting exclusions via `public HashSet<Type> ExcludedTypes` (currently only resolved at start-up)
